...
public class AntMiner extends RuleClassifier {
	...
	CONFIG.set(COLONY_SIZE, 120);
	CONFIG.set(MAX_ITERATIONS, 3000);
	CONFIG.set(MINIMUM_CASES, 10);
	CONFIG.set(MAXIMUM_LIMIT, 50);
...
/*@Override
    public ClassificationModel train2(Dataset dataset) {
        System.out.println("*********************train2*********************");
	SequentialCovering seco = new SequentialCovering();
	return new ClassificationModel(seco.train2(dataset));
    } 
// Decision Tree 
public ClassificationModel DT (Dataset dataset) {
    System.out.println("*********************Decision Tree*********************");
SequentialCovering seco = new SequentialCovering();
return new ClassificationModel(seco.DT(dataset));
}
@Override
protected  Model trainStep2(Dataset dataset, Model model) {

System.out.println("***********************incrimental*************************");
	SequentialCovering seco = new SequentialCovering();
	return new ClassificationModel(seco.train2(dataset));   
	} */
@Override
   ...
   /* @Override
    protected Model train2(Dataset dataset) {
  System.out.println("***************************train2**********************************");
	SequentialCovering seco = new SequentialCovering();
	return new ClassificationModel(seco.train2(dataset));    }
    }
